...
from etl.gcs_to_gbq import task2_gcs_to_gbq
# from etl.summarize_table import task3_summarize_table
...
with DAG(
    ...
    task_gcs_to_gbq = PythonOperator(
        task_id='gcs_to_gbq',
        python_callable= task2_gcs_to_gbq  # Chạy hàm từ file gcs_to_gbq.py
    )

    # # Định nghĩa task thứ ba: chạy summarize_table.py
    # task_summarize_table = PythonOperator(
    #     task_id='create_summarize_table',
    #     python_callable= task3_summarize_table  # Chạy hàm từ file summarize_table.py
    # )
